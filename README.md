...
> 详细架构图、agent 注入、节点/服务边界、生产路径请见 [`docs/architecture/DESIGN.md`](docs/architecture/DESIGN.md)。
> 本节是高层概览，DESIGN.md 是代码真相源。

```
用户输入（一句话前提）
        │
        ▼
   Director：ScenePlan 编排
        │
        ▼
   WorldBuilder：世界骨架扩写（首次）
        │
        ▼
   LangGraph 双循环（7 节点：director / scene_director /
   actor / gate_keeper / node_detector / narrator / world_builder）

   actor_node 内部（注意：Critic + GM 不是 graph 节点，是 factory 注入）：
       per-character isolated Actor LLM → 产出 ActionIntent
              → Critic.review_batch() (1×LLM)
              → GM.settle_scene()    (1×LLM) → SceneScript

   actor_node 输出 → gate_keeper_node → node_detector_node
                  → narrator_node（基于 narration_service）
        │
        ▼
...
> Critic / GM 是 agent 类（`src/worldbox_writer/agents/`），但**不是 LangGraph graph 节点**——它们以 factory 形式注入 `actor_node`。Narrator 已迁移到 `engine/services/narration_service.py`（Sprint 26 删除 `narrator.py`）。

| Agent            | 职责                                            |
| :--------------- | :-------------------------------------------- |
| **Director**     | 解析用户意图，初始化世界骨架，产出 ScenePlan 驱动每幕推演            |
| **WorldBuilder** | 扩写世界规则、势力、地理、力量体系，维护全局知识库                     |
| **Actor × N**    | 每个角色是独立 LLM 调用，产出 ActionIntent（由 `isolated_actor_service.run_isolated_actor_runtime` 调度）|
...
worldbox-writer/
├── src/worldbox_writer/
│   ├── agents/               # 7 个 Agent（Director / WorldBuilder / Actor / Critic / GateKeeper / GM / NodeDetector）
│   │   ├── director.py       # 意图解析与场景规划
│   │   ├── world_builder.py  # 世界扩写与知识库维护
│   │   ├── actor.py          # 单角色决策（legacy 路径；生产用 isolated_actor_service）
│   │   ├── critic.py         # 意图级审查
│   │   ├── gate_keeper.py    # 边界约束校验
│   │   ├── gm.py             # SceneScript 结算
│   │   └── node_detector.py  # 关键节点识别
│   │   # 注：narrator.py 已在 Sprint 26 删除，文本渲染改由 engine/services/narration_service.py 承担
│   ├── api/                  # FastAPI 后端（REST + SSE）
│   ├── core/                 # 核心模型与双循环契约
│   ├── engine/               # LangGraph StateGraph 推演引擎（7 节点：director / scene_director / actor / gate_keeper / node_detector / narrator / world_builder）
│   │   # 关键：Critic + GM 不是独立 graph 节点，而是 factory 注入到 actor_node 内部
│   │   # 详见 docs/architecture/DESIGN.md §5
│   ├── memory/               # 短期 / 长期 / 反思三层记忆
...
详见 [docs/sprints/SPRINT\_24.md](docs/sprints/SPRINT_24.md)。
...
历史 Sprint 规划（SPRINT\_0 到 SPRINT\_23）已归档至 `docs/archive/sprints/`，
不再作为维护入口。
...
- [开发指南（环境 / 命令 / CI / 分支提交 / 运行手册 / 发布 / 类型检查）](docs/development/DEVELOPMENT.md)
- [产品策略](docs/product/PRODUCT_STRATEGY.md)
- [质量评测系统 SPEC](docs/product/QUALITY_SPEC.md)
- [Orchestrator 总控手册（已归档）](docs/sprints/orchestrator/README.md)
