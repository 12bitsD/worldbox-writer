...
_OUTPUT_SCHEMA_BLURB = """输出严格 JSON，不要 markdown，不要解释。schema：
{
  ...
  "evidence_quote": "<单个连续原文片段，不超过 60 字，必须是文本中真实出现的字符串；找不到则空字符串>",
...
```json
{
  "applicable": true,
  "score": <1-10>,
  "evidence_quote": "<单个连续原文证据片段，不超过 60 字；score ≥ 5 必须非空>",
...
- 只要原文出现"哦，我的天"、"天哪"、"上帝啊"、"这真是"这类翻译腔整句，直接按 translation_tone 明显命中，score ≥ 8。
- 同一段对话里重复出现两次及以上"你或许不知道"、"实不相瞒"、"这一切都要从..."并说明大量背景，直接按 expository_dialogue 明显命中，score ≥ 8。
- evidence_quote 必须是文本里**真实存在**的字符串，禁止改写、总结、转述。
- evidence_quote 只能是一段连续原文，禁止拼接多个片段，禁止超过 60 字。
...
def build_user_message(text: str) -> str:
    return (
        f"待评测文本：\n---\n{text}\n---\n"
        "请只返回一行紧凑 JSON，不要 markdown/code fence/解释。\n"
        "evidence_quote 只能取单个连续原文片段，≤60 字；禁止拼接多个引文；"
        "如原文片段含双引号，改取不含双引号的相邻短片段。"
    )
