...
class ActorAgent:
    ...
    def _fallback_action_data(self, character: Character) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": (
                f"{character.name}在原地停住脚步，扫视与{goal}有关的可疑动静，"
                f"向最近的同伴低声确认下一步，不肯放弃{goal}。"
            ),
            "emotional_state": "谨慎",
            "consequence_hint": "局势暂时放缓，但角色目标仍在推进。",
        }

    def _ensure_visible_motivation(self, description: str, character: Character) -> str:
        if _description_has_visible_motivation(description, character):
            return description
        goal = character.goals[0] if character.goals else "当前目标"
        base = description.rstrip("。；;，, ")
        return f"{base}，不肯放弃{goal}。"


def _description_has_visible_motivation(description: str, character: Character) -> bool:
    text = description.strip()
    goals = [goal.strip() for goal in character.goals if goal.strip()]
    if any(goal and goal in text for goal in goals):
        return True
    motivation_markers = (
        "为了",
        "想要",
    ...
    return any(marker in text for marker in motivation_markers)
