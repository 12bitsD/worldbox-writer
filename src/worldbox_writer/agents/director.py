...
def derive_title_from_premise(premise: str, max_chars: int = 14) -> str:
    """Build a readable fallback title without cutting a phrase mid-way."""
    cleaned = "".join(premise.split()).strip("《》\"'“”")
    if not cleaned:
        return "《无名世界》"

    for mark in ("，", ",", "。", ".", "！", "!", "？", "?", "；", ";", "：", ":"):
        index = cleaned.find(mark)
        if 3 <= index <= max_chars + 4:
            return f"《{cleaned[:index]}》"

    if len(cleaned) <= max_chars:
        return f"《{cleaned}》"

    shortened = cleaned[:max_chars].rstrip("的与和、，,。.;；:：")
    return f"《{shortened or cleaned[:max_chars]}》"
...
class DirectorAgent:
    ...
    def _fallback_world_init_data(self, premise: str) -> Dict[str, Any]:
        ...
        return {
            "title": derive_title_from_premise(premise),
            "premise": premise,
