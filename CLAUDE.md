...
- `engine/dual_loop.py` + `core/dual_loop.py` implement the **dual-loop contract**, gated by `FEATURE_DUAL_LOOP_ENABLED`. The compare/report endpoint at `/api/simulate/{id}/dual-loop/compare` produces readiness reports and a rollback runbook (see `docs/development/DEVELOPMENT.md` §10).
...
- **`make typecheck` is informational, not a gate.** There is a documented mypy baseline in `docs/development/DEVELOPMENT.md` §11. Don't block unrelated work on existing errors, but don't add new ones either.
...
- `docs/architecture/DESIGN.md` — three-layer architecture, decision log, dual-loop first-principles derivation.
- `docs/development/DEVELOPMENT.md` — env vars, CI workflow map, layered testing strategy, runbook, feature-flag stop-loss, dual-loop rollout/rollback, release process, typecheck baseline.
