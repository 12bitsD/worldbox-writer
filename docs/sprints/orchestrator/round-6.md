...
**状态**：partial complete（R6.3 真实外部人工评分样本仍 pending；当前只有 proxy subset）
**分支**：`feature/sprint-25-r6-final-cleanup`；R6.3 续收口：`sprint-25-r6-external-calibration`
**主题**：Sprint 25 第二个 cleanup round。删 deprecated judge API + 迁移 e2e_judge.py + 外部 calibration scaffolding + CLAUDE.md / AGENTS.md / README / QUALITY_SPEC v1.0 对齐。真实外部人工评分样本仍是 R6 residual。
...
### 5.5 R6.3 external calibration proxy ✓ / 真实外部人工评分样本 △
...
说明：未提交任何真实长版权片段，也不冒充具体作者原文。该组是 Codex 原创的 external-style calibration proxy subset，用于验证 external fixture scaffolding 与 runner 能力。

**纠偏**：这组 proxy 不能等同于 Claude 要求的“外部人工评分样本”。Claude 原要求是“猫腻 / 烽火戏诸侯 / 起点头部 真实片段或精仿 + 人工排序入库”，核心目的是打破 AI 自循环偏差。Codex 原创 proxy 仍是 AI 生成样本，因此 R6-residual-1 不应关闭。真正完成仍需要人类提供授权片段、真实人工评分样本，或明确认可的人类精仿样本。
...
| (d) 外部样本 ≥ 3 段 | 3 段 Codex 原创 external-style proxy 已入库；真实外部人工评分样本仍 pending | △ |
| (e) 外部 ranking 验证 | proxy subset 的 `external_calibration_ranking.json`: Spearman 1.0（not gated）+ mandatory pair 0 反转 | ✓（proxy only） |
...
Real LLM external proxy calibration：3 samples × 3 runs × 15 dims = 135 underlying calls，duration 644.65s，排序 `X1_external_head_market` (6.773) > `X2_external_mid_common` (6.243) > `X3_external_ai_water` (0.0, 3/3 veto)，mandatory pair violations = 0，overall PASS。

结论：proxy scaffolding 通过，但 R6.3 的真实外部人工评分样本硬底线未完成，不能据此宣告 external calibration 风险关闭。
...
- `tests/test_evals/fixtures/calibration_v1/external/` —— 3 段 external-style proxy calibration subset + manifest（不等同真实外部人工评分样本）
...
- `artifacts/eval/sprint-25/round-6/external_calibration_ranking.json` —— external proxy subset real LLM ranking 报告

### 测试结果

- L1：211 passed / 57 deselected，无回退；R6.3 追加单测 2 passed
- 真实 LLM：external proxy calibration 135 underlying calls，mandatory pairs 0 反转，PASS；真实外部人工评分样本未完成
...
| R6 | final cleanup | 删 ~600 行 deprecated + 重写 e2e_judge + external proxy subset + 文档 v1.0（真实外部人工样本仍 pending） |
