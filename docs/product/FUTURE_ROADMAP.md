...
在成功交付 v0.4.0 MVP（端到端推演引擎、边界干预系统、可视化沙盒面板）之后，WorldBox Writer 已经证明了"沙盒式多 Agent 协作"在长篇小说生成中的可行性。
...
- **关系图谱可视化 (Social Graph Visualization)**：在前端集成动态的网络拓扑图（如 ECharts/React Flow），实时展示角色间的好感度、敌对关系及势力范围变化。
...
### 3.1 工作流与生产力增强
- **持久化工作区 (Workspace Persistence)**：将后端的内存状态持久化到数据库（如 SQLite/PostgreSQL），支持多项目管理、自动保存和断点续传。
- **富文本编辑器集成**：在 `Narrator` 渲染完成后，提供一个类似 Notion 的所见即所得编辑器，允许用户手动润色，并将修改后的关键信息反向同步给 `MemoryManager`。
...
- **真实 Agent 遥测 (Live Telemetry)**：将后台各 Agent 的"思考过程"（如 Actor 的提议被 GateKeeper 拒绝的原因）以弹幕或控制台日志的形式在前端优雅展示，增加"世界在运转"的真实感。
...
- **v0.5.0 (Persistence & Editor)**：实现数据库持久化，支持多项目管理；集成前端富文本编辑器。
- **v0.6.0 (Branching & Visuals)**：实现 DAG 分支推演和回溯功能；上线动态人物关系图谱。
- **v0.7.0 (Deep Worldbuilding)**：开放交互式设定集，支持导入外部大纲和设定。
- **v1.0.0 (Production Ready)**：完成性能优化和多模型路由，支持千万字级别的长篇小说稳定推演与导出。
