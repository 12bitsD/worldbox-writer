...
### 5.5 R6.3 外部人工评分样本（**跳过给 Codex**）

由于本轮额度收紧，外部人工样本入库 + ranking 验证留给 Codex 接力。详见 §下一轮预选题。
...
| (d) 外部样本 ≥ 3 段 | **跳过给 Codex** | △ |
| (e) 外部 ranking 验证 | **跳过给 Codex** | △ |
| (f) QUALITY_SPEC v1.0 + 文档对齐 | spec v1.0；state.json current_phase 切换 | ✓ |

L1 测试：211 passed（删除 8 deprecated tests + 新增 5 = 净 -3，无回退）。
...
- L1：211 passed / 57 deselected，无回退
- 真实 LLM：本轮无 LLM 调用（纯 cleanup）
...
| R6 | final cleanup | 删 ~600 行 deprecated + 重写 e2e_judge + 文档 v1.0 |
